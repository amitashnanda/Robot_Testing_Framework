...
class MoveToTray2:
    ...
    def movearm2Gear(self,data):
        ...
        if data.points[0].positions[0] == 0.0 and data.points[0].positions[1] == 3.14 and data.points[0].positions[2] == -1.570 and data.points[0].positions[3] == 2.14 and data.points[0].positions[4] == 3.27 and data.points[0].positions[5] == -1.51 and data.points[0].positions[6] == 0.0 and self.flag == 0:
            self.flag = 1
            self.arm_controller([[p1,t1]])
        
    
    def listenGripperState2(self):
        while not rospy.is_shutdown():
...
if __name__ == '__main__':
    try:
        rospy.init_node("put_gear_on_tray_2", log_level=rospy.DEBUG)
        rate = rospy.Rate(1)
    except:
        pass
    initRoboArm = MoveToTray2()
    initRoboArm.listenGripperState2()
