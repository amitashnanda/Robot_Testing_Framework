...
class MoveToTray4:
    ...
    def movearm2Gear(self,data):
        ...
        if data.points[0].positions[0] == 1.0 and data.points[0].positions[1] == 1.507 and data.points[0].positions[2] == 0 and data.points[0].positions[3] == -0.38 and data.points[0].positions[4] == 0.38 and data.points[0].positions[5] == -1.51 and data.points[0].positions[6] == 0.00:
            self.arm_controller([[p3,t3]])
        
    
        
    def listenGripperState4(self):
        while not rospy.is_shutdown():
...
if __name__ == '__main__':
    try:
        rospy.init_node("put_gear_on_tray_4", log_level=rospy.DEBUG)
        rate = rospy.Rate(1)
    except:
        pass
    initRoboArm = MoveToTray4()
    initRoboArm.listenGripperState4()
